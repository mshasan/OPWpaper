...
#'
#' @description \code{OPWeight} package proposed methods to compute the ranks
#' probabilities of filters given the test effect size and the corresponding
#' weights. This funciton uses the simulation results of the probabilities and
#' the weights to plot.
#'
#' @param ey_index Integer vector, column numbers of the null proportions of a
#' specific effect
#' @param null_index Integer vector, column numbers of the effects of a specific
#' null proportion
#' @param m Integer, total number of tests
#' @param ey Numeric, mean filter effect size used in \code{ey_index}
...
prob_weight_plots <- function(ey_index, null_index, m, ey, null, prob, weight)
{
    ...
    prob_by_null = data.frame(ranks, cbind(SMA(p_null[,1],n=100), SMA(p_null[,2],n=100),
                                           SMA(p_null[,3], n=100), SMA(p_null[,4], n=100)))
    ...
    prob_by_effect <- data.frame(ranks, cbind(SMA(p_ef[,1],n=100), SMA(p_ef[,2],n=100), SMA(p_ef[,3], n=100)))
    ...
    weight_by_null = data.frame(ranks, cbind(SMA(w_null[,1],n=100), SMA(w_null[,2],n=100),
                                             SMA(w_null[,3], n=100), SMA(w_null[,4], n=100)))
    ...
    weight_by_effect <- data.frame(ranks, cbind(SMA(w_ef[,1],n=100), SMA(w_ef[,2],n=100), SMA(w_ef[,3], n=100)))
