...
cont_probs_plots <- lapply(nullSize, prob_plots, effectType = "continuous")
bin_probs_plots <- lapply(99, prob_plots, effectType = "binary")
...
#prob <- read.csv("dataBin_ProbRanksByEffect_m10000.csv",h=T)[,26]
...
grid.arrange(p_prob, p_weight, ncol = 2,
                      top="Continuous: et = ey = 1, m = 10000, m0 = 9000, m1 = 1000")

# for supplementry materials------
grid.arrange(p_prob, p_weight, ncol = 2,
                      top="Continuous: et = ey = 1, m = 10000, m0 = 5000, m1 = 5000")

grid.arrange(p_prob, p_weight, ncol = 2,
                      top="Continuous: et = ey = 1, m = 10000, m0 = 9900, m1 = 100")
...
simuVal = 1:1000
typeIerror_mat = sapply(cl, alphaVal, simu_Type_I_error, m = 10000, simu = 1000)


# load saved data---------------------
# this code is to load saved workspace from parallel computing
load("U:/Documents/My Research (UGA)/Multiple Hypoetheses/Article-1/smu_Type_I_error.RDATA")
...
alphaVal = seq(.01, .1, .02)
datError <- data.frame(alphaVal, t(typeIerror_mat))
...
fwerPowerFdrPower_by_effect <- function(filterEffect, m , null, testCorr=0,
                                        random=0, groupSize=100, alpha=.05, Sigma)
  {
    ...
    FWER_pro <- sum(rej_pro[OD[,3] == 0])		# FWER of proposed method
    FWER_bon <- sum(rej_bon[dat[,3] == 0])			# FWER of bonferroni method
    FWER_rdw <- sum(rej_rdw[dat[,3] == 0])			# FWER of Roeder Wasserman method
    FWER_ihw <- sum(rej_ihwFwer[dat[,3] == 0])			# FWER of IHW method
...
#-------------------------------------------------
dat_50 <- data.frame(effectVec, t(FwerPowerFdrPower2e1[13:16,]))
...
dat_90 <- data.frame(effectVec, t(FwerPowerFdrPower4e1[13:16,]))
...
dat_99 <- data.frame(effectVec, t(FwerPowerFdrPower5e1[13:16,]))
...
# plots of power for 
# mean test effect(et) ~ Normal (mean filter effect, mean filter effect/2)
#----------------------------------------------------------------------------
dat_50 <- data.frame(effectVec, t(FwerPowerFdrPower2e2[13:16,]))
colnames(dat_50) <- c("effectSize", "PRO", "BH", "RDW", "IHW")
dat_50_all <- melt(dat_50, id.var = "effectSize")
p_50_all <- ggplot(dat_50_all, aes(x = effectSize, y = value,group = variable,
                             col=variable)) +
...
dat_90 <- data.frame(effectVec, t(FwerPowerFdrPower4e2[13:16,]))
colnames(dat_90) <- c("effectSize", "PRO", "BH", "RDW", "IHW")
dat_90_all <- melt(dat_90, id.var = "effectSize")
p_90_all <- ggplot(dat_90_all, aes(x = effectSize, y = value,group = variable,
                                   col=variable)) +
...
dat_99 <- data.frame(effectVec, t(FwerPowerFdrPower5e2[13:16,]))
colnames(dat_99) <- c("effectSize", "PRO", "BH", "RDW", "IHW")
dat_99_all <- melt(dat_99, id.var = "effectSize")
p_99_all <- ggplot(dat_99_all, aes(x = effectSize, y = value,group = variable,
                                   col=variable)) +
...
# see correaltion effect on Power
#-----------------------------------------
# load data for continuous power----------------
# this code is to load saved workspace from parallel computing
load("U:/Documents/My Research (UGA)/Multiple Hypoetheses/Article-1/simu_Continuous_FwerPowerFdrPower.RDATA")
...
dat_50 <- data.frame(effectVec, t(FwerPowerFdrPower2e1[1:4,])*m0)
colnames(dat_50) <- c("effectSize", "PRO", "BH", "RDW", "IHW")
dat_50_all <- melt(dat_50, id.var = "effectSize")
p_50_all <- ggplot(dat_50_all, aes(x = effectSize, y = value,group = variable,
                             col=variable)) +
...
m0 = 9000
dat_90 <- data.frame(effectVec, t(FwerPowerFdrPower4e1[1:4,])*m0)
colnames(dat_90) <- c("effectSize", "PRO", "BH", "RDW", "IHW")
dat_90_all <- melt(dat_90, id.var = "effectSize")
p_90_all <- ggplot(dat_90_all, aes(x = effectSize, y = value,group = variable,
                                   col=variable)) +
...
m0 = 9900
dat_99 <- data.frame(effectVec, t(FwerPowerFdrPower5e1[1:4,])*m0)
colnames(dat_99) <- c("effectSize", "PRO", "BH", "RDW", "IHW")
dat_99_all <- melt(dat_99, id.var = "effectSize")
p_99_all <- ggplot(dat_99_all, aes(x = effectSize, y = value,group = variable,
                                   col=variable)) +
...
# plots FWER et ~ Normal(ey, ey/2)
#-------------------------------------------------
m0 = 5000
dat_50 <- data.frame(effectVec, t(FwerPowerFdrPower2e2[1:4,])*m0)
colnames(dat_50) <- c("effectSize", "PRO", "BH", "RDW", "IHW")
dat_50_all <- melt(dat_50, id.var = "effectSize")
p_50_all <- ggplot(dat_50_all, aes(x = effectSize, y = value,group = variable,
                             col=variable)) +
...
m0 = 9000
dat_90 <- data.frame(effectVec, t(FwerPowerFdrPower4e2[1:4,])*m0)
colnames(dat_90) <- c("effectSize", "PRO", "BH", "RDW", "IHW")
dat_90_all <- melt(dat_90, id.var = "effectSize")
p_90_all <- ggplot(dat_90_all, aes(x = effectSize, y = value,group = variable,
                                   col=variable)) +
...
m0 = 9900
dat_99 <- data.frame(effectVec, t(FwerPowerFdrPower5e2[1:4,])*m0)
colnames(dat_99) <- c("effectSize", "PRO", "BH", "RDW", "IHW")
dat_99_all <- melt(dat_99, id.var = "effectSize")
p_99_all <- ggplot(dat_99_all, aes(x = effectSize, y = value,group = variable,
                                   col=variable)) +
...
# plots FDR et = ey
#-------------------------------------------------
dat_50 <- data.frame(effectVec, t(FwerPowerFdrPower2e1[9:12,]))
colnames(dat_50) <- c("effectSize", "PRO", "BH", "RDW", "IHW")
dat_50_all <- melt(dat_50, id.var = "effectSize")
p_50_all <- ggplot(dat_50_all, aes(x = effectSize, y = value,group = variable,
                             col=variable)) +
...
dat_90 <- data.frame(effectVec, t(FwerPowerFdrPower4e1[9:12,]))
colnames(dat_90) <- c("effectSize", "PRO", "BH", "RDW", "IHW")
dat_90_all <- melt(dat_90, id.var = "effectSize")
p_90_all <- ggplot(dat_90_all, aes(x = effectSize, y = value,group = variable,
                                   col=variable)) +
...
dat_99 <- data.frame(effectVec, t(FwerPowerFdrPower5e1[9:12,]))
colnames(dat_99) <- c("effectSize", "PRO", "BH", "RDW", "IHW")
dat_99_all <- melt(dat_99, id.var = "effectSize")
p_99_all <- ggplot(dat_99_all, aes(x = effectSize, y = value,group = variable,
                                   col=variable)) +
...
# plots FDR et ~ Normal(ey, ey/2)
#-------------------------------------------------
dat_50 <- data.frame(effectVec, t(FwerPowerFdrPower2e2[9:12,]))
colnames(dat_50) <- c("effectSize", "PRO", "BH", "RDW", "IHW")
dat_50_all <- melt(dat_50, id.var = "effectSize")
p_50_all <- ggplot(dat_50_all, aes(x = effectSize, y = value,group = variable,
                             col=variable)) +
...
dat_90 <- data.frame(effectVec, t(FwerPowerFdrPower4e2[9:12,]))
colnames(dat_90) <- c("effectSize", "PRO", "BH", "RDW", "IHW")
dat_90_all <- melt(dat_90, id.var = "effectSize")
p_90_all <- ggplot(dat_90_all, aes(x = effectSize, y = value,group = variable,
                                   col=variable)) +
...
dat_99 <- data.frame(effectVec, t(FwerPowerFdrPower5e2[9:12,]))
colnames(dat_99) <- c("effectSize", "PRO", "BH", "RDW", "IHW")
dat_99_all <- melt(dat_99, id.var = "effectSize")
p_99_all <- ggplot(dat_99_all, aes(x = effectSize, y = value,group = variable,
                                   col=variable)) +
...
# load data for continuous power----------------
# this code is to load saved workspace from parallel computing
load("C:/UGA/PhD research/Binary_misVariance_FwerPowerFdrPower.RDATA")
...
#----------------------------------------------------------------------------
dat_50_cv1 <- data.frame(effectVec, t(FwerPowerFdrPower1a2[13:16,]))
...
dat_50_cv3 <- data.frame(effectVec, t(FwerPowerFdrPower1a3[13:16,]))
...
dat_50_cv10 <- data.frame(effectVec, t(FwerPowerFdrPower1a5[13:16,]))
...
dat_50_cv1_par <- data.frame(effectVec[1:6], t(FwerPowerFdrPower1a2[13:16,1:6]))
...
dat_50_cv3_par <- data.frame(effectVec[1:6], t(FwerPowerFdrPower1a3[13:16,1:6]))
...
dat_50_cv10_par <- data.frame(effectVec[1:6], t(FwerPowerFdrPower1a5[13:16,1:6]))
...
# 90% null case------------
dat_90_cv1 <- data.frame(effectVec, t(FwerPowerFdrPower2a2[13:16,]))
...
dat_90_cv3 <- data.frame(effectVec, t(FwerPowerFdrPower2a3[13:16,]))
...
dat_90_cv10 <- data.frame(effectVec, t(FwerPowerFdrPower2a5[13:16,]))
...
dat_90_cv1_par <- data.frame(effectVec[1:6], t(FwerPowerFdrPower2a2[13:16,1:6]))
...
dat_90_cv3_par <- data.frame(effectVec[1:6], t(FwerPowerFdrPower2a3[13:16,1:6]))
...
dat_90_cv10_par <- data.frame(effectVec[1:6], t(FwerPowerFdrPower2a5[13:16,1:6]))
