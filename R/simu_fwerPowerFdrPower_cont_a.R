...
fwerPowerFdrPower_by_effect <- function(filterEffect, m , null, testCorr=.2, random=0, groupSize=100, alpha=.05) 
{
  ...
  Sigma <- matrix(testCorr, groupSize, groupSize) + diag(groupSize)*(1-testCorr)		# test correlation matrix
  
  group = 1:mGrp
  
  filter <- if(testCorr == 0) {rnorm(m, ef, 1)
  } else {as.vector(sapply(group, test_by_block, eVec=ef, groupSize=groupSize, Sigma = Sigma))}	# filter test stat
  
  test <- if(testCorr == 0) {rnorm(m, et, 1)
  } else {as.vector(sapply(1:mGrp, test_by_block, eVec=et, groupSize=groupSize, Sigma=Sigma))}	# actual test stat
  ...
  ranks = 1:m
  ranksProb <-sapply(ranks, prob_rank_givenEffect, et = abs(ey_cont),
                     ey = abs(ey_cont), nrep = 10000, m0 = m0, m1 = m1)
  
  ranksProb <- ranksProb/sum(ranksProb)
  ...
  #----------------------------------------------------------------------------------------
  weight_rdw <- as.vector(rw_weight(testStat = OD$test, gamma=.05, alpha=alpha, group=5, tail=2))		# roeder wasserman weight
  ihw_fwer <- ihw(OD$pval, OD$filter, alpha=alpha, adjustment_type = "bonferroni")		# IHW method for FWER
  ihw_fdr <-  ihw(OD$pval, OD$filter, alpha=alpha, adjustment_type = "BH")			# IHW method for FDR
  ...
  FWER_pro <- sum(rej_pro[OD$et == 0])/max(1, sum(OD$et == 0))			# FWER of proposed method
  FWER_bon <- sum(rej_bon[OD$et == 0])/max(1, sum(OD$et == 0))			# FWER of bonferroni method
  FWER_rdw <- sum(rej_rdw[OD$et == 0])/max(1, sum(OD$et == 0))			# FWER of Roeder Wasserman method
  FWER_ihw <- sum(rej_ihwFwer[OD$et == 0])/max(1, sum(OD$et == 0))			# FWER of IHW method
...
FwerPowerFdrPower_simu_a <- parSapply(cl, simuVal, simu_fwerPowerFdrPower_by_effect, filterEffect=effecSize, m = mVal, null=.5, testCorr=0, random=0, groupSize = 100, alpha = .05)
FwerPowerFdrPower_simu_b <- parSapply(cl, simuVal, simu_fwerPowerFdrPower_by_effect, filterEffect=effecSize, m = mVal, null=.9, testCorr=0, random=0, groupSize = 100, alpha = .05)
FwerPowerFdrPower_simu_c <- parSapply(cl, simuVal, simu_fwerPowerFdrPower_by_effect, filterEffect=effecSize, m = mVal,null=.99, testCorr=0, random=0, groupSize = 100, alpha = .05)
...
save.image("simu_fwerPowerFdrPower_cont_a.RData")
